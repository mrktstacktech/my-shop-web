...
export function Card(props: CardType) {
    ...
    const priceDiscounted = props.discountPercentage ? props.price * (1 - props.discountPercentage / 100) : props.price;
    const likeIcon = (
    ...
    return (
        <>
            <div onMouseOver={() => setIsHovered(true)}
                ...
                className="relative bg-(--item-background-color) flex items-center justify-center h-48 w-48 p-0 rounded-sm mb-2" 
            >
                <div
                ...
                <img className={isHovered ? `w-18` : `w-20`} src={props.thumbnail} alt={props.title} />
                
            </div>
            <h2 className="font-medium mt-1 mb-1">{props.title}</h2>
            <div className="text-sm"><span className="text-red-500">${priceDiscounted}</span> <span className="line-through text-gray-400">${props.price}</span></div>
        </>
    )
}